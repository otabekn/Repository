...
namespace DapperRepository
{
    ...
    public sealed class DynamicQuery
    {
        ...
        public static string GetInsertQuery(string tableName, dynamic item)
        {
            ...
            var list = props.Where(x => x.GetCustomAttributes(typeof(ComputedAttribute), true).Length ==0).ToList();

            string[] columns = list.Select(p => p.Name).Where(s => s.ToLower() != "id").ToArray();
        ...
        public static string GetUpdateQuery(string tableName, dynamic item)
        {
            ...
            string[] columns = list.Where(p => p.Name.ToUpper() != "ID").Select(p => p.Name).ToArray();
            //string[] columns = props.Select(p => p.Name).ToArray();
            var parameters = columns.Select(name => name + "=@" + name).ToList();

            string result = string.Format("UPDATE {0} SET {1} WHERE ID=@ID", tableName, string.Join(",", parameters));
            return result;
        }
